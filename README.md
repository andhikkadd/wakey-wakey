...
1. **Copy Build Files:**
   Copy the contents of the `dist/` folder to a permanent folder on your PC (e.g. `C:\Program Files\WakeyWakey\` or `C:\Users\<Name>\AppData\Local\WakeyWakey\`).
...
```text
WakeyWakey/
├── WakeyWakey.sln            # Solution file containing WakeyWakey project
├── .gitignore                # Target build folder and local config exclusions
├── LICENSE                   # Open-source MIT license
├── src/
│   └── WakeyWakey/           # WPF Project folder
│       ├── Models/
│       │   ├── Alarm.cs              # Alarm configuration & DaysOfWeek flags
│       │   └── AlarmDisplayModel.cs  # Wrapper for WPF binding representation
│       ├── Services/
│       │   ├── AlarmStorage.cs       # Load/Save configs from %APPDATA%
...
│       └── WakeyWakey.csproj         # MSBuild project file
